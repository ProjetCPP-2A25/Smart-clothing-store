...
/*bool Employe::ajouter() {
    ...
    QSqlQuery query;
    query.prepare("INSERT INTO EMPLOYEE (CIN, NOM_E, PRENOM_E, POST, EMAIL, TELEPHONE, ADRESSE) "
                  "VALUES (:CIN, :NOM_E, :PRENOM_E, :POST, :EMAIL, :TELEPHONE, :ADRESSE)");

    // Log input values
    qDebug() << "Inserting values:";
    qDebug() << "CIN:" << CIN << "NOM_E:" << NOM_E << "PRENOM_E:" << PRENOM_E
             << "POST:" << POST << "EMAIL:" << EMAIL << "TELEPHONE:" << TELEPHONE
             << "ADRESSE:" << ADRESSE;
...
/*bool Employe::supprimerCONG(int CIN)
{
    QSqlQuery query;
        query.prepare("UPDATE EMPLOYEE "
                      "SET FIRST_DAY_OF_LEAVE = NULL, "
                      "    LAST_DAY_OF_LEAVE = NULL, "
                      "    LEAVE_TYPE = NULL, "
                      "    NBDAYS = NULL "
                      "WHERE CIN = :CIN");
        query.bindValue(":CIN", CIN);
...
/*bool Employe::Modifier(QString NOM_E, QString PRENOM_E, QString POST, QString EMAIL, int TELEPHONE, QString ADRESSE, QString LEAVE_TYPE,QDate FIRST_DAY_OF_LEAVE,
                       QDate LAST_DAY_OF_LEAVE,int NBDAYS)
{
    QSqlQuery query;
    QString cin_string = QString::number(CIN); // Use the instance variable CIN

    query.prepare("UPDATE EMPLOYEE SET NOM_E= :NOM_E, PRENOM_E= :PRENOM_E, POST= :POST, EMAIL= :EMAIL, TELEPHONE= :TELEPHONE, ADRESSE= :ADRESSE WHERE CIN= :CIN;");
    query.bindValue(":NOM_E", NOM_E);
    query.bindValue(":PRENOM_E", PRENOM_E);
    query.bindValue(":POST", POST);
    query.bindValue(":EMAIL", EMAIL);
    ...
    query.bindValue(":ADRESSE", ADRESSE);
    query.bindValue(":CIN", cin_string); // Use the CIN to identify the record

    if (!query.exec()) {
        qDebug() << "Error executing query:" << query.lastError().text(); // Debugging line
        return false;
    }
...
/*void Employe::stat_bateau() {
    ...
    for (int i = 0; i < model->rowCount(); i++) {
        QString typeBateau = model->record(i).value("LEAVE_TYPE").toString();
